...
An autonomous [Vercel Eve](https://eve.dev/) agent that watches YouTube RSS feeds, downloads new-video MP3 audio with RapidAPI, stores the file path in Supabase, transcribes locally with `faster-whisper`, creates a concise narrative summary with DeepSeek v4 Flash, and sends it to Telegram.
...
2. Install server tools: `ffmpeg` and Python `faster-whisper`.
3. Copy `.env.example` to `.env.local` and fill in the Supabase service-role key, your DeepSeek API key, RapidAPI key, Telegram bot/chat details, and optional Apify API token. Keep `.env.local` private.
...
- Transcript creation downloads MP3 audio through RapidAPI first, saves it under `AUDIO_DIR`, then transcribes locally. Direct YouTube captions are used as backup. If those fail and `APIFY_API_TOKEN` is set, it runs Apify Actor `zPumutvB61fpEsglh` as the final fallback.
