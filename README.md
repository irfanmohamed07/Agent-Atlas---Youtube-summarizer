...
2. Copy `.env.example` to `.env.local` and fill in the Supabase service-role key, your OpenAI API key, Telegram bot/chat details, and optional Apify API token. Keep `.env.local` private.
...
- `agent/schedules/retry_transcripts.md` retries unavailable captions every 5 hours for about 24 hours, then tries Apify once per day for two more days before marking the video failed.
...
- Transcript fetching tries direct YouTube captions first. If those fail and `APIFY_API_TOKEN` is set, it runs Apify Actor `zPumutvB61fpEsglh` as a fallback.
