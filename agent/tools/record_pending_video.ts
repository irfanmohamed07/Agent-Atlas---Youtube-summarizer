...
export default defineTool({
  ...
  inputSchema: z.object({ channelId: z.number().int().positive(), videoId: z.string(), title: z.string(), videoUrl: z.string().url(), publishedAt: z.string(), retryCount: z.number().int().min(1).max(FINAL_RETRY_LIMIT), audioPath: z.string().nullable().optional(), failureReason: z.string().nullable().optional() }),
  outputSchema: z.object({ status: z.enum(["PENDING", "FAILED"]), retryCount: z.number().int() }),
  async execute({ channelId, videoId, title, videoUrl, publishedAt, retryCount, audioPath, failureReason: providedFailureReason }) {
    ...
    const failureReason = providedFailureReason || (failed ? "Transcript unavailable after 24 hours and 2 daily Apify attempts" : "Transcript unavailable; retry scheduled");
