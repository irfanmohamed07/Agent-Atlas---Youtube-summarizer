...
export default defineTool({
  ...
  async execute(input) {
    await supabase("videos?on_conflict=video_id", {
      ...
      body: JSON.stringify({ channel_id: input.channelId, video_id: input.videoId, title: input.title, video_url: input.videoUrl, published_at: input.publishedAt, transcript: input.transcript, summary: input.summary, processed: true, status: "PROCESSED", transcript_retry_count: 0, next_transcript_retry_at: null, failure_reason: null, processed_at: new Date().toISOString() }),
    });
    return { saved: true };
  },
});
