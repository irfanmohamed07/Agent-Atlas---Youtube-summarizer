...
export default defineTool({
  ...
  inputSchema: z.object({ channelId: z.number().int().positive(), videoId: z.string(), title: z.string(), videoUrl: z.string().url(), publishedAt: z.string(), transcript: z.string(), summary, audioPath: z.string().nullable().optional() }),
  ...
  async execute(input) {
    ...
    await supabase("videos?on_conflict=video_id", {
      ...
      body: JSON.stringify({ channel_id: input.channelId, video_id: input.videoId, title: input.title, video_url: input.videoUrl, published_at: input.publishedAt, transcript: input.transcript, summary: input.summary, processed: false, status: "PENDING", transcript_retry_count: 0, next_transcript_retry_at: null, failure_reason: null, processed_at: null, ...(input.audioPath ? { audio_path: input.audioPath } : {}) }),
    });
